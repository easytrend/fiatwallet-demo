...
async function fetchLiveRates() {
  ...
  let fetchedFiat = false;
  let fetchedCrypto = false;

  for (const api of FIAT_APIS) {
    try {
      const r = await fetchWithTimeout(api.url, { timeout: 5000 });
  ...
  try {
    ...
    const r = await fetchWithTimeout(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd`, { timeout: 5000 });
    if (!r.ok) throw new Error("HTTP " + r.status);
    const j = await r.json();
    Object.entries(COINGECKO_IDS).forEach(([sym, id]) => {
      if (j[id]?.usd) {
        result.crypto[sym] = j[id].usd;
        fetchedCrypto = true;
      }
    });
  } catch(e) {  }
  ...
  if (!result.crypto.SOL) {
    try {
      const r = await fetchWithTimeout("https://api.coinbase.com/v2/prices/SOL-USD/spot", { timeout: 5000 });
      if (r.ok) {
        const j = await r.json();
  ...
  if (!result.crypto.SOL) {
    try {
      const r = await fetchWithTimeout("https://api.binance.com/api/v3/ticker/price?symbol=SOLUSDT", { timeout: 5000 });
      if (r.ok) {
        const j = await r.json();
  ...
  if (!result.crypto.SOL) {
    try {
      const r = await fetchWithTimeout("https://api.jup.ag/price/v2?ids=So11111111111111111111111111111111111111112", { timeout: 5000 });
      if (r.ok) {
        const j = await r.json();
  ...
  if (!fetchedFiat && !fetchedCrypto) {
    throw new Error("Failed to fetch both fiat and crypto rates");
  }
